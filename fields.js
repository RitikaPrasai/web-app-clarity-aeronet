...
import {updateAOD, updateTime, getStartEndDateTime, getChartDate} from './components.js';
import {initDropdown, initDropdownCN, initMap} from './init.js';
import { MarkerManager } from "./marker.js";
import { resetClarity, resetOpenAQ, setBaseMap } from './map.js';
...
export class FieldInitializer {
    ...
    init() {
        ...
        const datatypeOpt = [{value: 20, label: 'Daily'},{value: 10, label: 'Near Real Time'}];
        ...
        const inactiveOff = `<div class="tooltip-container label-show-inactive">
        ...
        const submitButtonDiv = `<div class="tooltip-container">
                                    <div id='row'><label for="site-drop-down">&nbsp;</label></div>
                                    <div class="input-container">
        ...
        fieldsContainer.innerHTML = `<form style="display:flex">${inactiveOff}${dropdownAOD}${dropdownSite}${dropdownData}${calender}${submitButtonDiv}</form>`;

        // Append the fields container to the map container
        const mapContainer = document.getElementById('tab-container');
    ...
    setChartStart() {
        return getChartDate(this.markerLayer.chartTimeLength, this.dateTime);
    ...
    updateDataSources(){
        var date = this.dateTime[0][0]+"-"+this.dateTime[0][1]+"-"+this.dateTime[0][2];
        
        // resetClarity();
        
        
        resetOpenAQ(date,true);
        resetClarity(date,true);
        this.normalizeMarkers();
